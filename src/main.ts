...
const countdown = getDate(new Date(1970, 0, 8, 23, 55, 41));
...
document.addEventListener(
	...
	() => {
		...
		timer = setInterval(() => {
			...
			updateHours(hours);
			updateDays(days);
			timerScreenReaderElem?.setAttribute(
				'aria-label',
				`Time left to launch: ${days} days ${hours} hours ${minutes} minutes`
			);
		}, 1000);
	},
	false
...
function updateCountdownTimeElem(id: string) {
	...
	const digitsCard: DigitsCard = {
		...
		bottomDigitsElem: document.querySelector(`#${id} .countdown__digits--bottom`),
	};
	let renderedTime = 0;
	return function updateTime(newTime: number) {
		toggleFlipCard({ newTime: newTime, oldTime: renderedTime }, flipCard, digitsCard);
		renderedTime = newTime;
	};
}

function toggleFlipCard(time: Time, flipCard: FlipCard, digitsCard: DigitsCard) {
	if (
	...
	if (time.newTime !== time.oldTime) {
		...
		setTimeout(() => {
			if (flipCard.topElem && flipCard.bottomElem && digitsCard.bottomDigitsElem) {
				flipCard.topElem.textContent = newTimeStr;
				flipCard.bottomElem.textContent = newTimeStr;
...
function calculateTimeLeft(countdown: Date, timer: number): (timePassed: number) => UnitsOfTime {
